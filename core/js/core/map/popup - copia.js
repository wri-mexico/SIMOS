define(['OpenLayers','timer','cluster'],function(OL,Timer,Cluster) {
        ...
        setData:function(){
            ...
            if(typeof(a.prev) == 'object'){
                 if(a.prev.tabla.indexOf('denue')!=-1){
                        ...
                        Cluster.showRecordCard({id:a.prev.gid,lonlat:lonlat,xy:{x:a.px.x,y:a.px.y}});
                 }else{
    ...
    $.widget( "custom.popup", {
                ...
                options:{
                    viewOrder:['top-left','top-right','bottom-left','bottom-right','left-top','left-bottom','right-rop','right-bottom'],
                    timer:null,
                    clock:null,
                    showOn:null,
                    title:'Titulo',
                    ...
                    for(var x in order){
                        switch(order[x]){
                            case 'top-left'://bien
                                ...
                                $("#area").css({top:coords.top+'px',left:coords.left+'px',height:popup.height+'px',width:popup.width+'px',display:'','z-index':50000})
